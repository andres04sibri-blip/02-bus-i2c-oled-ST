...
bool initDisplay() {
    ...
    if (!display.begin(SSD1306_SWITCHCAPVCC, OLED_I2C_ADDR)) {
        ...
        Serial.println(
            "[OLED] ERROR CRÍTICO: No responde el display en 0x3C."
        );
...
void showSystemReady() {
    ...
    Serial.println(
        "\n[SISTEMA] Auto-diagnóstico superado con éxito. "
        "Estación operativa."
    );
}
...
void setup() {
    ...
    // 2. Verificar dispositivos y pantalla OLED
    if (total > 0 && initDisplay()) {
        ...
        // Reportar falla
        Serial.println(
            "\n[BOOT] ERROR: No se pudo completar "
            "la inicialización del sistema."
        );

        Serial.println(
            "[DIAGNÓSTICO] Verifique las conexiones I2C, "
            "alimentación y dirección de la OLED."
        );
    }
}
